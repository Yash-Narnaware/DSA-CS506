...
int mom3(int *arr, int arr_size1,int lb,int ub) {
    ...
    int number_of_subarrays = (arr_size + 2)/3;
    ...
    for (i = 0; i < arr_size/3; i++) {
        ...
        for (int j = 0; j < 3; j++){
            cpy_arr2[j] = arr[lb + i * 3 + j];
        }
        medians[i] = findmedian(cpy_arr2,3);
    }

    if (arr_size%5 != 0){
        int cpy_arr3[arr_size%3];

        for (int j = 0; j < arr_size%3; j++){
            cpy_arr3[j] = arr[lb + i * 3 + j];
        }
        medians[i] = findmedian(cpy_arr3, arr_size%3);
        i++;
    }
    ...
    mom3(medians,number_of_subarrays,0,number_of_subarrays-1);
...
void quicksort(int *list1,int size,int lb,int ub){
    ...
    if (lb < ub){
        ...
        int piviot = mom3(list1,size,lb,ub);
        // printf("pivot: %d\n",piviot);
        // return;
