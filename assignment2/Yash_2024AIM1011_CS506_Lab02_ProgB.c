...
//function to find the median of 5 or less number of elements 
...
int mom(int *arr, int arr_size1,int lb,int ub) {
    ...
    //dividing array in size of 5
    int number_of_subarrays = (arr_size + 4)/5;

    int medians[number_of_subarrays];

    int i = 0;
    for (i = 0; i < arr_size/5; i++) {
        int cpy_arr2[5];

        for (int j = 0; j < 5; j++){
            cpy_arr2[j] = arr[lb + i * 5 + j];
        }
        medians[i] = findmedian(cpy_arr2,5);
    }

    if (arr_size%5 != 0){
        int cpy_arr3[arr_size%5];

        for (int j = 0; j < arr_size%5; j++){
            cpy_arr3[j] = arr[lb + i * 5 + j];
        }
        medians[i] = findmedian(cpy_arr3, arr_size%5);
        i++;
    }

    mom(medians,number_of_subarrays,0,number_of_subarrays-1);
...
void quicksort(int *list1,int size,int lb,int ub){
    ...
    if (lb < ub){
        ...
        int piviot = mom(list1,size,lb,ub);
